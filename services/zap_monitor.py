"""Deprecated shim: payment coordinator and diagnostics.

This module historically mixed Nostr event handling and payment logic. Today:
- Realtime zaps and engagement are handled by the WebSocket monitor and
  subscriptions pipeline (kinds 9735, 6, 7).
- The invoice listener is used strictly to trigger automatic split payouts.

Migration path:
- Prefer services/payment_coordinator.py for new code. It exposes
  PaymentCoordinatorService and get_payment_coordinator, which delegate to
  this implementation during the transition.

This file remains to preserve imports like get_zap_monitor and
ZapMonitorService for existing callers and admin/testing flows.
"""
